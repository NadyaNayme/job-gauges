...
export const setDefaultSettings = (): void => {
  const settings = document.querySelectorAll<HTMLInputElement>('[data-setting]');
...
export const loadSettings = (): void => {
  ...
  for (const dataSetting of settings) {
    if (dataSetting.dataset.setting === undefined) throw Error(`Setting (${dataSetting.id}) is missing a data-setting`);
