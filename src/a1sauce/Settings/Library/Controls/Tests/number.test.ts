...
const testNumber = <HTMLInputElement>(
    createNumberSetting('testNumber', 'This is a test dropdown', {
        defaultValue: 50,
        min: 0,
        max: 100,
    })
...
describe('Library.Controls.createNumberSetting()', () => {
    ...
    test('Should have default values for min, max, and defaultValue if no options are passed', () => {
        expect(
            testNumberNoOptions.querySelector('input').getAttribute('min'),
        ).toEqual('1');
        expect(
            testNumberNoOptions.querySelector('input').getAttribute('max'),
        ).toEqual('20');
