...
describe('Patches.serverCheck()', () => {
    test('Github semver is behind Current semver', async () => {
        ...
        expect(parseInt(res.version.split('.')[0], 10)).toBeLessThanOrEqual(
            majorVersion,
        );
        expect(parseInt(res.version.split('.')[1], 10)).toBeLessThanOrEqual(
            minorVersion,
        );
        expect(parseInt(res.version.split('.')[2], 10)).toBeLessThanOrEqual(
            patchVersion,
        );
    });
