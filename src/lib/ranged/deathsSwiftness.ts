...
export async function deathsSwiftnessOverlay(gauges: Overlay) {
	...
	if (!deathsSwiftness.active) {
		handleAbilityActiveState(abilityData, 'DeathsSwiftness', !deathsSwiftness.isOnCooldown);
	...
	if (lastValue !== deathsSwiftness.time) {
		...
		alt1.overLayTextEx(`${deathsSwiftness.time || ''}`,
			white,
			14,
			adjustPositionForScale(ranged.position.x + active_orientation.x + 26, gauges.scaleFactor),
			adjustPositionForScale(ranged.position.y + active_orientation.y + 26, gauges.scaleFactor),
			3000,
			'',
			true,
			true
